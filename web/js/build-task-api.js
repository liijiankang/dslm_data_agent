...
function sourceById(sourceId) {
  return window.MockData.dataSources.find((source) => source.id === sourceId) || window.MockData.dataSources[0];
}
...
function previewBuildTask(payload) {
  const response = window.MockData.buildTaskPreviewResponses[payload.parseStrategy] || window.MockData.buildTaskPreviewResponses.知识;
  const cloned = clone(response);
  const source = sourceById(payload.sourceId);
  const assets = window.MockData.buildTaskPreviewAssets[payload.parseStrategy] || window.MockData.buildTaskPreviewAssets.知识;
...
function createBuildTask(payload) {
  const source = sourceById(payload.sourceId);
  const previewAssets = clone(window.MockData.buildTaskPreviewAssets[payload.parseStrategy] || window.MockData.buildTaskPreviewAssets.知识);
  const requestedIds = payload.buildScope === "all" || payload.assetIds === null ? null : new Set(payload.assetIds);
  ...
  const response = clone(window.MockData.buildTaskCreateResponse);
